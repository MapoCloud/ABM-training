...
to setup                                                                        ;; build the world
  ...
  create-turtles 500 [                                                             ;; create 500 agents, ask each to do the following
    ...
    set wealth random-normal 100 20
...
;###################################################################              ;; add investment
  if investment [ ask turtles with [wealth > 90] [set wealth wealth + wealth * r] ]
;###################################################################
...
to transact                                                                       ;; transact wealth
  ;#################################################################################################
  if taxtype = "default" [set wealth wealth - 1 set t 0 ask one-of other turtles [ set wealth wealth + 1 ]]
  if taxtype = "proportional" [set wealth wealth - wealth * taxrate set t wealth * taxrate]
  if taxtype = "progressive" [ifelse wealth <= 50 [set wealth wealth * 1][ifelse wealth < 150 [set wealth wealth - wealth * taxrate set t wealth * taxrate][set wealth wealth - wealth * (taxrate + 0.01) set t wealth * (taxrate + 0.01)]]]
  let tax sum [t] of turtles
  let poverty count turtles with [wealth < 50]
  if poverty > 0 [set transferpayment tax / poverty]
  if wealth <= 50 [set wealth wealth + 1]
...
taxtype
taxtype
"default" "proportional" "progressive"
2
