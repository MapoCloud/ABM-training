...
to go                                                                            ;; running the world
  ...
  ;################################################################################################# 此处开始修改,加入投资及投资门槛
   ; if investment = 1 [
    ;ask turtles [if wealth > 200 [set wealth wealth * r] ] ;; 设定投资门槛，满足投资门槛后可以获得收益
    ;]
  ;################################################################################################# 加入经济增长
    ;if growth = 1 [
    ;if growthtype = "propoor" [
    ;if ]
  ;]
  ;
  ;
...
to transact                                                                       ;; transact wealth
  ;######################################################################################################## 税收及转移支付
  if taxtype = "default" [set wealth wealth - 1 ask one-of other turtles [ set wealth wealth + 1 ]]
  if taxtype = "proportional" [set wealth wealth - wealth * taxrate / 100 set t wealth * taxrate / 100]
  if taxtype = "progressive" [if wealth < 50 [set wealth wealth * 1]
                              if wealth < 200 [set wealth wealth - wealth * taxrate / 100 set t wealth * taxrate / 100]
                              if wealth = 200 [set wealth wealth - wealth * taxrate / 100 set t wealth * taxrate / 100]
                              if wealth > 200 [set wealth wealth - wealth * (taxrate + 1) / 100] set t wealth * (taxrate + 1) / 100]
  ;########################################################################################################
